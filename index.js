...
function render(resume) {
  ...
  _(resume.projects).forEach((project_info) => {
    ...
    if (can_calculate_period) {
      project_info.duration =
        project_info.endDate != null && end_date.isValid()
    ...
    project_info.highlights = _(project_info.highlights).map((highlight) =>
      convertMarkdown(highlight)
    );
  });
  ...
  _(resume.work).forEach((work_info) => {
    ...
    if (can_calculate_period) {
      work_info.duration =
        work_info.endDate != null && end_date.isValid()
          ? moment.preciseDiff(start_date, end_date)
          : moment.preciseDiff(start_date, moment());
    }
    ...
    work_info.highlights = _(work_info.highlights).map((highlight) =>
      convertMarkdown(highlight)
    );
  });
